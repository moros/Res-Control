//
//  DisplayModelMenuItem.m
//  Res Switcher
//
//  Created by Doug Mason on 1/2/21.
//
#import "DisplayModelMenuItem.h"
...
+ (NSArray *)getMenuItemsForDisplay:(CGDirectDisplayID)display {
    ...
    for (int i = 0; i < numberOfDisplayModes; i++) {
        ...
        if (previousMenuItem == nil || mode.depth > previousMenuItem.mode.depth) {
            if (mode.modeNumber == currentDisplayModeNumber || previousMenuItem.state == NSControlStateValueOn) {
            ...
            [previousMenuItem setState:NSControlStateValueOn];
        }
    }
